...
function pega_tipo_contrato2($valor) {
	...
	else $retorno="<span class=\"vermelho\">Voluntário</span>";
...
function pega_turno_padrao($i) {
	...
	if ($i!='l') {
		$vetor[-3]= "Costura";
		$vetor[-2]= "Plantão Domingo";
		$vetor[-1]= "Plantão Sábado";
	}
...
function pega_passo_percurso($i) {
	switch ($i) {
		case 1: $tipo= "Saída da empresa"; break;
		case 2: $tipo= "Passagem em cliente"; break;
		case 3: $tipo= "Retorno à empresa"; break;
		default: $tipo=""; break;
	}
	return($tipo);
}
...
function pega_tipo_detalhamento_relatorio($tipo) {
	if ($tipo==1) return("Sintético");
	else return("Analítico");
}
...
function bate_ponto($id_funcionario, $data_batida, $hora, $tipo, $id_supervisor, $hl, $vale_dia, $turnante) {
	...
	if ($result_ponto) {
		...
		if ($data_aniversario==date("d/m", $arruma)) {
			$mostra .= "
			<div id=\"informacao_batida_aniversario\"><blink>FELIZ ANIVERSÁRIO!!!</blink></div><br /><br />";
		...
		$mostra .= "<div id=\"nome_funcionario\">
			<ul>
				...
				<li><strong>Horário registrado:</strong> ". $hora ."</li>
				$li_turnante
			</ul>
		</div>
...
function pega_funcionarios_trabalhando_retroativo($id_empresa, $id_departamento, $id_turno_index, $vale_dia, $data_hora_entrada, $data_hora_saida, $identificar, $horas) {
	...
	for ($i=0; $i<$horas; $i++) {
		...
		if ($identificar=="1") echo "<br /> Funcionários trabalhando entre <strong>$min2f</strong> e <strong>$max1f</strong> : <br />";
		...
		while ($rs= mysql_fetch_object($result)) {
			...
			if (($rs->id_cargo=="10") || ($rs->id_funcionario=="14")) {
				$trabalhou="0";
				$pre= "<strong>SUPERVISOR - NÃO CONSIDERANDO... </strong>";
			}
			else {
				$trabalhou="1";
				$pre= "";
...
function pega_jornada_diaria($id_turno, $id_dia) {
	...
	//intervalo total entra o inicio e o fim da jornada diária
...
function pega_duracao_intervalo_dia($id_turno, $id_dia) {
	...
	if (mysql_num_rows($result_intervalo)>0) {
		...
		//jornada diária, descontando o intervalo, descontando 4 x 10min
			...
			if ($linhas_teste==0) {
				...
				if (mysql_num_rows($result_destinatarios)>0) {
					...
					while ($rs_destinatarios= mysql_fetch_object($result_destinatarios)) {
						...
						$corpo_mensagem= "Olá <strong>". $rs_destinatarios->nome_rz ."</strong>,
											só para avisar que o funcionário <strong>". $rs->nome_rz ."</strong> do setor <strong>". pega_departamento($id_depto_carreira) ."</strong>, turno <strong>". pega_turno($id_turno_carreira) ."</strong>
											está fazendo aniversário no dia <strong>". $rs->dia ."/". $rs->mes ."</strong>.<br /><br />";
						
						$retorno= enviar_mensagem($_SESSION["id_empresa"], 0, $rs_destinatarios->id_pessoa, "Aniversário de funcionário - <strong>". $rs->nome_rz ."</strong> em <strong>". $rs->dia ."/". $rs->mes ."</strong> ". $periodo, $corpo_mensagem);
						if (!$retorno) $var++;
						
						//echo "Aniversário de <strong>". $rs->nome_rz ."</strong> em <strong>". $rs->data2 ."</strong> ". $periodo;
					}
				}
			}
		}
			...
			if ($linhas_teste==0) {
				...
				if (mysql_num_rows($result_destinatarios)>0) {
					...
					while ($rs_destinatarios= mysql_fetch_object($result_destinatarios)) {
						...
						$corpo_mensagem= "Olá <strong>". $rs_destinatarios->nome_rz ."</strong>,
											só para avisar que <strong>". $rs->nome ."</strong> (". pega_tipo_pessoa($rs->tipo_pessoa) ." &raquo; <strong>". $rs->nome_rz ."</strong> &raquo; Cargo <strong>". $rs->cargo ."</strong> do setor <strong>". $rs->setor ."</strong>)
											está fazendo aniversário no dia <strong>". $rs->dia ."/". $rs->mes ."</strong>.<br /><br />";
						
						$retorno= enviar_mensagem($_SESSION["id_empresa"], 0, $rs_destinatarios->id_pessoa, "Aniversário de ". pega_tipo_pessoa($rs->tipo_pessoa) ." - <strong>". $rs->nome ."</strong> em <strong>". $rs->dia ."/". $rs->mes ."</strong> ". $periodo, $corpo_mensagem);
						if (!$retorno) $var++;
...
function alerta_documentos($id_empresa) {
	...
	while ($rs_dc= mysql_fetch_object($result_dc)) {
		...
		if ($dias<=$rs_dc->alerta_dias) {
			...
			$corpo_mensagem= "Olá ". pega_pessoa($id_pessoa) .", este é um lembrete enviado pelo sistema solicitado por você.<br /><br />
				O documento <strong>". $rs_dc->documento ."</strong> está vencendo no dia <strong>". $rs_dc->data_vencimento2 ."</strong>, faltam <strong>". $rs_dc->alerta_dias ."</strong> dia(s) ou menos para esta data.<br /><br />
			...
			$retorno= enviar_mensagem($_SESSION["id_empresa"], 0, $id_pessoa, "Aviso de vencimento de documento cód. ". $rs_dc->id_documento, $corpo_mensagem);
...
function pega_escolaridade($tipo) {
	...
	$vetor[3]= "Ensino médio incompleto";
	$vetor[4]= "Ensino médio completo";
	$vetor[5]= "Curso superior incompleto";
	$vetor[6]= "Curso superior completo";
	$vetor[7]= "Mestrado";
	$vetor[8]= "Doutorado";
	$vetor[9]= "Nível técnico incompleto";
	$vetor[10]= "Nível técnico completo";
...
function xpega_acompanhamento_atividades($tipo) {
	...
	$vetor[1]= "Presença dos funcionários";
	$vetor[2]= "Leitura do livro eletrônico";
	$vetor[3]= "Visualização do peso de roupa suja";
	$vetor[4]= "Visualização do peso de roupa limpa";
	$vetor[5]= "Montagem de kits de uniformes";
	$vetor[6]= "Controle de consumo";
	$vetor[7]= "Registro de costura";
	$vetor[8]= "Verificação da pressão de vapor";
	$vetor[9]= "Manutenção da organização da lavanderia";
	$vetor[10]= "Ginástica laboral";
	$vetor[11]= "Agendamento das visitas";
	$vetor[12]= "Passagem de plantão";
...
function pega_manutencao_checklist_categorias($tipo) {
	...
	$vetor[3]= "Caldeira";
	$vetor[4]= "Carrinhos";
	$vetor[5]= "Casa de máquinas";
	$vetor[6]= "Compressor";
	$vetor[7]= "Lavadoras";
	$vetor[8]= "Manutenção predial";
	$vetor[9]= "Secadora";
...
function pega_situacao_rm($tipo) {
	...
	$vetor[1]= "Manutenção solicitada";
	$vetor[2]= "Em análise";
	$vetor[3]= "Em andamento";
	$vetor[4]= "Aguardando peças";
	$vetor[5]= "Finalizada";
...
function pega_situacao_reclamacao($tipo) {
	...
	$vetor[1]= "Ação corretiva";
	$vetor[2]= "Ação preventiva";
	$vetor[3]= "Melhoria de processo";
	$vetor[4]= "Sugestão";
	$vetor[5]= "Investigação";
	$vetor[6]= "Finalizada";//5 //update qual_reclamacoes_andamento set id_situacao= '6' where id_situacao = '5'
	$vetor[7]= "Reclamação reaberta";//6 //update qual_reclamacoes_andamento set id_situacao= '7' where id_situacao = '6'
	$vetor[8]= "Não solucionada";
...
function pega_tipo_combustivel($tipo) {
	...
	$vetor[4]= "Álcool";
	$vetor[5]= "Gás natural";
	$vetor[6]= "Óleo para motor";
	$vetor[7]= "Óleo hidráulico";
...
function pega_motivo_costura($tipo) {
	...
	$vetor[1]= "Dano químico";
	$vetor[2]= "Dano mecânico";
...
function pega_pecas_roupa($id_peca) {
	...
	$vetor[1]= "Avental";
	$vetor[2]= "Avental cirúrgico";
	$vetor[3]= "Calça";
	$vetor[4]= "Camisola";
	$vetor[5]= "Campo cirúrgico grande";
	$vetor[6]= "Campo cirúrgico pequeno";
	...
	$vetor[19]= "Toalha pequena de mão";
	$vetor[20]= "Touca";
	$vetor[21]= "Cobertor";
...
function pega_itens_devolucao($tipo) {
	...
	$vetor[1]= "Res. biológicos";
	$vetor[2]= "Bacia";
	$vetor[3]= "Inst. cirúrgico";
	$vetor[4]= "Papagaio";
	$vetor[5]= "Comadre";
	$vetor[6]= "Cubeta";
...
function pega_acao_carreira($tipo) {
	...
	$vetor[3]= "Mudança de departamento/cargo";
	$vetor[4]= "Mudança de turno";
	/*$vetor[4]= "Contrato de experiência (30 dias)";
	$vetor[5]= "Contrato de experiência (mais 60 dias)";
	$vetor[6]= "Efetivado(a)";*/
...
function pega_checklist_admissao($tipo) {
	...
	$vetor[3]= "Contrato de experiência";
	$vetor[4]= "Registro funcional";
	$vetor[5]= "Vale transporte";
	$vetor[6]= "Cartão de identificação";
	$vetor[7]= "Uniforme";
	$vetor[8]= "Ordem de serviço";
	$vetor[9]= "Armário";
	$vetor[10]= "EPI";
...
function pega_checklist_demissao($tipo) {
	...
	$vetor[1]= "Comprovante de pagamento de rescisão";
	$vetor[2]= "Rescisão (5 vias)";
	$vetor[3]= "Pedido/aviso de demissão";
	$vetor[4]= "Extrato do FGTS";
	$vetor[5]= "Atestado de saúde demissional";
	$vetor[6]= "Comprovação de férias";
	$vetor[7]= "Registro de empregado";
	$vetor[8]= "Carteira de trabalho e previdência social";
	$vetor[9]= "Aviso prévio";
...
function checa_versao_contatos($tipo, $id_empresa, $id_usuario) {
	...
	//se o ultimo registro encontrado nao for o que está fazendo, insere o que está fazendo e incrementa a versão
...
function pega_turno($id_turno) {
	if ($id_turno==-1) return("PLANTÃO SÁBADO");
	elseif ($id_turno==-2) return("PLANTÃO DOMINGO");
...
function calcula_horario_intervalo($tipo, $id_intervalo, $id_dia, $data) {
	...
	$retorno .= "". $horario_inicio ." às ". $horario_fim ." ";
...
function retorna_intervalo_diurno_noturno($datac1, $datac2, $datac1_anterior, $datac2_anterior) {
	...
	//se entrou e saiu no horário diurno
	if ( (eh_diurno($hora1[0])) && (eh_diurno($hora2[0])) && ($hora1[0]<=$hora2[0]) ) {
		//se tem mais de uma operacao no dia, se as operacoes anteriores forem válidas...
		if (($datac1_anterior!="") && ($datac2_anterior!="")) {
			//se a hora que entrou anteriormente é noturno e...
			//a diferenca entre a entrada atual e a saída anterior for menor ou igual a 30min
			//... é uma continuacao de hora extra
	...
	elseif ( (eh_diurno($hora1[0])) && (eh_diurno($hora2[0])) && ($hora1[0]>$hora2[0]) ) {
		...
		if ( ((eh_diurno($hora1[0])) && (eh_noturno($hora2[0]))) || ((eh_diurno($hora1[0])) && ( ($hora2[0]==4) || ($hora2[0]==5) || ($hora2[0]==6) ) ) ) {
			...
			if ( (eh_noturno($hora1[0])) && (eh_diurno($hora2[0])) ) {
				...
				//se entrou e saiu no horário noturno
...
function calcula_diurno_noturno($entrada, $saida) {
	...
	for ($i=0; $i<4; $i++) {
		if ( ($entrada[$i]!="") && ($saida[$i]!="") ) {
			...
			//echo "<strong>entrada $i</strong>: ". $entrada[$i] ." | <strong>saída $i</strong>: ". $saida[$i] ."<br>";
...
function pega_operacao_debito($operacao, $operacao_debito) {
	...
	elseif ($operacao_debito==0) return("Mudança de horário/folga");
...
function pega_tipo_afastamento($i) {
	...
	switch ($i) {
		...
		case 'p': $tipo= "Perícia"; break;
		case 'o': $tipo= "Outros abonos"; break;
		case 'f': $tipo= "Férias"; break;
		case 'd': $tipo= "Advertência"; break;
		case 's': $tipo= "Suspensão"; break;
...
function pega_tipo_motivo($i) {
	switch ($i) {
		...
		case 'd': $tipo= "Advertência"; break;
		case 's': $tipo= "Suspensão"; break;
		case 'p': $tipo= "Alteração no ponto"; break;
		case 't': $tipo= "Descontos"; break;
		case 'r': $tipo= "Refeições"; break;
		case 'q': $tipo= "Saída no estoque"; break;
...
function pega_analise_agua_tratada($tipo) {
	...
	$vetor[6][0]= "pH";
	$vetor[6][1]= "6,5 a 9,5";
	$vetor[7][0]= "Cloro livre (após carvão)";
	$vetor[7][1]= "0,1 ppm";
...
function pega_equipamento_local($tipo) {
	...
	$vetor[1]= "Área de O.R e pré-tratamento";
	$vetor[2]= "Elementos do sistema";
	$vetor[3]= "Quadro elétrico";
	$vetor[4]= "Bomba(s) de alimentação";
	$vetor[5]= "Filtro(s) de areia";
	$vetor[6]= "Abrandador(es)";
	$vetor[7]= "Filtro(s) de carvão";
	$vetor[8]= "Osmose reversa";
	$vetor[9]= "Bomba(s) de recirculação";
	$vetor[10]= "Tanque(s) e sistema de distribuição";
...
function pega_ba_comp($tipo) {
	...
	$vetor[1]= "Componentes hidráulicos";
	$vetor[2]= "Componentes elétricos";
...
function pega_fa_comp($tipo) {
	...
	$vetor[1]= "Sistema hidráulico";
	$vetor[2]= "Sistema elétrico";
	$vetor[3]= "Programação e retrolavagem";
...
function pega_ab_comp($tipo) {
	...
	$vetor[2]= "Sistema elétrico";
	$vetor[3]= "Programação e retrolavagem";
	$vetor[4]= "Nível de NaCl no tanque";
...
function pega_or_itens1($tipo) {
	...
	$vetor[1]= "Pressão de entrada do pré-filtro (PSI)";
	$vetor[2]= "Pressão de saída do pré-filtro (PSI)";
	$vetor[3]= "Pressão da bomba da OR (PSI)";
	$vetor[4]= "Vazão da água tratada (gpm)";
	$vetor[5]= "Vazão da água rejeitada (gpm)";
	$vetor[6]= "Recuperação (%)";
	$vetor[7]= "Rejeição salina (%)";
	$vetor[8]= "Temperatura (ºC)";
...
function pega_br_comp($tipo) {
	...
	$vetor[1]= "Componentes hidráulico";
	$vetor[2]= "Componentes elétrico";
...
function pega_ts_comp($tipo) {
	...
	$vetor[1]= "Componentes hidráulico";
	$vetor[2]= "Componentes elétrico";
	$vetor[3]= "Regulador de nível do tanque";
...
function pega_condicoes_operacionais($tipo) {
	...
	$vetor[1]= "Pressão de entrada do pré-filtro (PSI)";
	$vetor[2]= "Pressão de saída do pré-filtro (PSI)";
	$vetor[3]= "Pressão da bomba de OR (bar)";
	$vetor[4]= "Pressão do concentrado (bar)";
	$vetor[5]= "Vazão do produto/permeado (GPM)";
	$vetor[6]= "TDS - quantidade sólidos dissolvidos (ppm)";
	$vetor[7]= "Pressão da bomba de alimentação (PSI)";
	$vetor[8]= "Temperatura (ºC)";
	$vetor[9]= "Recuperação";
	$vetor[10]= "Rejeição salina";
...
function pega_vt_equipamentos($tipo) {
	...
	$vetor[1]= "Higiene";
	$vetor[2]= "Quadro elétrico";
	$vetor[3]= "Bomba(s) de alimentação";
	$vetor[4]= "Filtro(s) de areia";
	$vetor[5]= "Abrandador(es)";
	$vetor[6]= "Filtro(s) de carvão";
	$vetor[7]= "Osmose reversa";
	$vetor[8]= "Bomba(s) de recirculação";
	$vetor[9]= "Tanque de armazenamento";
	$vetor[10]= "Sistema de distribuição";
...
function pega_fl_itens($tipo) {
	...
	$vetor[3]= "Área de trabalho";
	$vetor[4]= "Janela acrílica";
	$vetor[5]= "Bandeja para coleta de resíduos";
	$vetor[6]= "Cabo externo";
	$vetor[7]= "Painel eletrônico";
	$vetor[8]= "Luminárias";
	$vetor[9]= "Lâmpada fluorescente";
	$vetor[10]= "Lâmpada germicida";
	$vetor[11]= "Dreno";
	$vetor[12]= "Porta de acesso aos filtros";
	$vetor[13]= "Pré-filtro";
	$vetor[14]= "Proteção do filtro";
	$vetor[15]= "Filtro absoluto de insuflamento";
	$vetor[16]= "Filtro absoluto de exaustão";
	$vetor[17]= "Tomada auxiliar";
	$vetor[18]= "Quadro elétrico";
	$vetor[19]= "Conj. motor de ventilador";
	$vetor[20]= "Corrente do motor";
	$vetor[21]= "Capacitor";
	$vetor[22]= "Variador de velocidade";
	$vetor[23]= "Contador";
	$vetor[24]= "Rele térmico";
	$vetor[25]= "Reator";
	$vetor[26]= "Start";
	$vetor[27]= "Fusíveis";
	$vetor[28]= "Botoeira";
	$vetor[29]= "Polias";
	$vetor[30]= "Correias";
	$vetor[31]= "Registro para gás";
	$vetor[32]= "Registro para ar";
	$vetor[33]= "Registro para água";
	$vetor[34]= "Manômetro";
...
function pega_fl_parametros($tipo) {
	...
	$vetor[1][0]= "Classificação da área";
	$vetor[1][1]= "5";
	
	$vetor[2][0]= "Designações da classe (partículas/m³)";
	$vetor[2][1]= "1 - 100";
	...
	$vetor[4][1]= "1 minuto minucioso para todas as partículas";
	
	$vetor[5][0]= "Fator de conversão";
...
function entrada_saida($tipo) {
	...
	else return("<span class=\"vermelho\">Saída</span>");
}

function entrada_saida_erro($tipo) {
	if ($tipo==1) return("<span class=\"verde\">Entrada</span>");
	elseif ($tipo==0) return("<span class=\"verde\">Saída</span>");
...
function envia_email_senha($id_empresa) {
	...
	$corpo= "<b>". data_extenso() ."</b>
					<br /><br />
					Olá <b>". $rs->contato ."</b>, a empresa <b>". $rs->nome_fantasia ."</b> foi adicionada no sistema online da Prospital.
					<br /><br />
					...
					<br /><br />
					Esta confirmação é importante para que a senha se torne privada, ou seja, somente você ou o responsável legal por sua entidade conhecerão a senha de acesso ao sistema!
					Logo após o cadastro da senha não é possível recuperá-la, somente cadastrar uma nova.
					<br /><br />
					------ <br />
					Atenciosamente,
					<br /><br />
	...
	enviar_email("prospital@prospital.com", "Prospital.com | Confirmação de senha", $corpo);
	//enviar_email($rs->email, "Prospital.com | Confirmação de senha", $corpo);
}
...
function pega_tipo_coleta($tipo) {
	switch($tipo) {
		case "m": $retorno= "Exame microbiológico"; break;
		case "f": $retorno= "Exame físico-químico"; break;
		default: $retorno= ""; break;
	}
	return($retorno);
}
...
function pega_tipo_atendimento($tipo_atendimento) {
	switch($tipo_atendimento) {
		case "i": $retorno= "Instalação"; break;
		case "m": $retorno= "Manutenção"; break;
