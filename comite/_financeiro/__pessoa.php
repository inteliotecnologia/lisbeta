...
if (pode_algum("2iaprz", $_SESSION["permissao"])) {
...
<form action="<?= AJAX_FORM; ?>formPessoa&amp;acao=<?= $acao; ?>" enctype="multipart/form-data" method="post" name="formPessoa" id="formPessoa" onsubmit="return ajaxForm('<?=$div_alvo;?>', 'formPessoa', 'validacoes', 1);">
	...
	<? } elseif ($tipo_pessoa=='x') { ?>
    <input name="tipo" class="escondido" type="hidden" id="tipo" value="c" />
    <? } else { ?>
    ...
    <div id="tipo_pessoa_atualiza">
        <?
        if ( (($acao=='i') || ($rs->tipo=='j')) && ($tipo_pessoa!='x') ) require_once("_financeiro/__pessoaj.php");
    ...
    <fieldset>
        ...
        <div class="parte50">
            ...
            <div id="id_cidade_atualiza">
              ...
              <?
                ...
                $result_cid= mysql_query("select * from cidades where id_uf = '". $id_uf ."' order by id_cidade");
				$i=0;
                while($rs_cid= mysql_fetch_object($result_cid)) {
                ?>
              <option <? if ($i%2==0) echo "class=\"cor_sim\""; ?>  value="<?= $rs_cid->id_cidade; ?>" <? if ($rs_cid->id_cidade==$rs->id_cidade) echo "selected=\"selected\""; ?> <? if ( ($rs_cid->id_cidade==8458) && ($acao=='i') ) echo "selected=\"selected\""; ?>><?= $rs_cid->cidade; ?></option>
              <? $i++; } ?>
            </select>
            </div>
            <br />
            ...
            ?>
            <option <? if ($i%2==0) echo "class=\"cor_sim\""; ?> value="<?= $rs_uf->id_uf; ?>" <? if ($rs_uf->id_uf==$rs->id_uf) echo "selected=\"selected\""; ?> <? if ( ($rs_uf->id_uf==24) && ($acao=='i') ) echo "selected=\"selected\""; ?>><?= $rs_uf->uf; ?></option>
            <? $i++; } ?>
            </select>
            <br />
        ...
        <div class="parte50">
            ...
            <?
            if ($acao=='i') $telefone= "(48) ";
            else $telefone= $rs->telefone;
            ?>
            
            <label for="telefone">Telefone:</label>
            <input name="telefone" id="telefone" value="<?= $telefone; ?>" onkeypress="return formataCampo(form, this.name, '(99) 9999-9999', event);" maxlength="14" />
            <br />
            <? /*} else { ?>
            <br />
