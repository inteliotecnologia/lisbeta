...
if (pode("rh", $_SESSION["permissao"])) {
	...
	if ($_GET["tipo"]=="2")
	$str .= " AND rh_funcionarios.oficial = '2' ";

	$str .= " order by pessoas.nome_rz asc ";
	
	$result_equipe= mysql_query("select DISTINCT(rh_funcionarios.equipe) as equipe from rh_funcionarios, pessoas, rh_carreiras, rh_enderecos
	...
	while ($rs_equipe= mysql_fetch_object($result_equipe)) {
		...
		$result = mysql_query("select * from rh_funcionarios, pessoas, rh_carreiras, rh_enderecos
								...
								AND   rh_funcionarios.status_funcionario = '1'
								AND   rh_carreiras.id_departamento = '29'
								AND   rh_funcionarios.equipe = '". $rs_equipe->equipe ."'
								". $str ."
								") or die(mysql_error());
		$linhas= mysql_num_rows($result);
		
		$pdf->SetFont('ARIAL_N_NEGRITO', '', 12);
		$pdf->Cell(0, 0.75, "CHAMADA - EQUIPE ". $rs_equipe->equipe, 0, 1, 'R');
		...
		while ($rs= mysql_fetch_object($result)) {
			...
			$pdf->Cell(4, 0.58, " ". str_replace("(48) ", "", $rs->telefone) ."/". $rs->bairro, 1, 0, 'L', $fill);
		...
		$result_local_equipe= mysql_query("select * from rh_funcionarios
												where equipe = '". $rs_equipe->equipe ."'
												and   local_equipe <> ''
												limit 1
												");
			$rs_local_equipe= mysql_fetch_object($result_local_equipe);
		
		$pdf->MultiCell(0, 0.6, $rs_local_equipe->local_equipe, 0, 'L');
