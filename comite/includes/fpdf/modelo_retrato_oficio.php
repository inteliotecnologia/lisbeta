...
class PDF extends FPDF {
	...
	function Header() {
		...
		if (file_exists(CAMINHO ."empresa_". $_SESSION["id_empresa_atendente2"] .".jpg"))
			$this->Image("". CAMINHO ."empresa_". $_SESSION["id_empresa_atendente2"] .".jpg", 14, 1.3, 5, 1.9287);
		
		$this->SetFont('ARIALNARROW', '', 8);
		
		if ($this->PageNo()>1) {
			$this->SetXY(2,3.2);
			$this->Cell(0, 0.3, "(Página ". $this->PageNo() ." do Ofício ". $_SESSION["oficio_num"] .", datado de ". $_SESSION["oficio_datado"] .")", 0, 1, 'R');
		}
		/*$this->SetXY(7,1.6);
	...
	function Footer() {
		...
		//echo ($this->pages[2]) ."<br /><br /><br /><br /><br /><br /><br />";
		//echo count($this->pages) ."<br />";
		
		
		
		if (($this->page==1) && ($_SESSION["oficio_num_paginas"]>1)) {
			$this->SetXY(2,26.75);
			$this->Cell(0, 0.3, "(Página ". $this->PageNo() ." do Ofício ". $_SESSION["oficio_num"] .", datado de ". $_SESSION["oficio_datado"] .")", 0, 1, 'R');
		}
		...
		$this->Cell(0, 0.4, "RELATÓRIO CRIADO POR ". VERSAO .", GERADO POR ". $_SESSION["nome"] ." EM ". date("d/m/Y"), 0, 1, 'C');
