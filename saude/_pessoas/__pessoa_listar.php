...
if (($_GET["todos"]==1) && ($_SESSION["tipo_usuario_sessao"]=="a")) {
	if ($_GET["repetidos"]=="")
		$sql= "select *, DATE_FORMAT(data_nasc, '%d/%m/%Y') as data_nasc from pessoas order by id_pessoa asc";
	else
		$sql= "select *, DATE_FORMAT(data_nasc, '%d/%m/%Y') as data_nasc
				from pessoas group by pessoas.nome, pessoas.id_pessoa
				having (select count(*)
				from pessoas cli
				where pessoas.nome = cli.nome) > 1
	...
	if ( (isset($txt_busca)) && (strlen($txt_busca)>2) ) {
		...
		$sql= "select *, DATE_FORMAT(data_nasc, '%d/%m/%Y') as data_nasc from pessoas ";
		switch ($lugar) {
			/*case 'todos': $sql .= "
