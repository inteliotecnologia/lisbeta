...
$result_con= mysql_query("select almoxarifadom_mov.*, pessoas.nome as funcionario, DATE_FORMAT(data_trans, '%d/%m/%Y %H:%i:%s') as data_trans from
							almoxarifadom_mov, usuarios, pessoas
							...
							and   almoxarifadom_mov.id_usuario = usuarios.id_usuario
							and   usuarios.id_pessoa = pessoas.id_pessoa
							order by almoxarifadom_mov.id_mov desc
							");
...
<h2 class="titulos" <?= $str_tit; ?>>Histórico completo de materiais - <?= pega_nome($id_pessoa_hist); ?> <? if ($cpf_mostra!="") echo formata_cpf($cpf_mostra); ?></h2>
...
<tr>
	...
	<td align="left"><?= pega_material($rs_con->id_material); ?></td>
